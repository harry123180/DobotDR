# -*- coding: utf-8 -*-
"""
ProcessMonitor.py - 多模組記憶體和執行緒監控工具
監控工業自動化專案中各個模組的資源使用情況
"""
...
class ProcessMonitor:
    ...
    def __init__(self, monitor_interval: int = 10):
        ...
        self.target_processes = {
            # 基礎服務
            'TCP-Server': ['TCPServer.py', 'ModbusServer'],
            
            # VP震動盤模組
            'VP-Main': ['VP_main.py', 'VP'],
            'VP-App': ['VP_app.py', 'VP'],
            
            # LED燈光模組
            'LED-Main': ['LED_main.py', 'light'],
            'LED-App': ['LED_app.py', 'light'],
            
            # 夾爪模組
            'Gripper-Main': ['Gripper.py', 'Gripper'],
            'Gripper-App': ['Gripper_app.py', 'Gripper'],
            
            # 視覺模組
            'CCD1-Vision': ['CCD1VisionCodeYOLO.py', 'CCD1'],
            'CCD3-Main': ['CCD3_main_app.py', 'CCD3'],
            
            # 機械臂模組
            'Dobot-Main': ['Dobot_main.py', 'M1Pro'],
            
            # 自動化流程
            'AutoFeeding': ['AutoFeeding_main.py', 'AutoFeeding'],
            'AutoProgram-Main': ['AutoProgram_main.py', 'AutoProgram'],
            'AutoProgram-App': ['AutoProgram_app.py', 'AutoProgram'],
        }
        ...
        self.logger.info("ProcessMonitor初始化完成")
        ...
        for proc in psutil.process_iter(['pid', 'name', 'cmdline', 'create_time']):
            try:
                ...
                for module_name, search_terms in self.target_processes.items():
                    ...
                    # 特殊處理：檢查視窗標題匹配 (對應啟動腳本中的title)
                    try:
                        if hasattr(proc, 'name') and 'cmd.exe' in proc_name.lower():
                            # 透過命令行參數中的conda activate ROBOT來確認是我們的進程
                            if 'conda activate ROBOT' in cmdline:
                                for search_term in search_terms:
                                    if search_term in cmdline:
                                        found = True
                                        break
                    ...
                    if found:
                        ...
                        # 詳細記錄找到的進程
                        self.logger.debug(f"找到模組 {module_name}: PID {proc_info['pid']}, CMD: {cmdline[:100]}...")
                        break  # 避免重複匹配
    ...
    def analyze_threads(self, module_name: str, threads: List[Dict]):
        ...
        if thread_count > self.thread_warning_count:
            self.logger.warning(
                f"執行緒數量過多 [{module_name}]: {thread_count}個執行緒"
            )
    ...
    def save_report(self, report: Dict):
        ...
        report_file = os.path.join(self.log_dir, f'monitor_report_{timestamp}.json')
    ...
    def print_summary(self, report: Dict):
        """打印監控摘要 - 按照實際模組分組顯示"""
        print("\n" + "="*80)
        print(f"📊 DobotCASE多模組監控報告 - {report['timestamp']}")
        print("="*80)
        ...
        while self.monitoring:
            try:
                ...
                if self._loop_count % 10 == 0:
                    ...
                    self.logger.info(f"已保存第 {self._loop_count} 次監控報告")
    ...
    def start_monitoring(self):
        ...
        self.logger.info(f"監控已啟動，間隔 {self.monitor_interval} 秒")
...
def main():
    ...
    parser = argparse.ArgumentParser(description='DobotCASE多模組進程監控工具')
    ...
    monitor = ProcessMonitor(monitor_interval=args.interval)
