...
def get_current_angle(self) -> AngleDetectionResult:
    """獲取當前角度值 (僅檢測，不校正)
    
    供Flow2調用，僅執行CCD3檢測獲取角度值，不進行馬達移動
    
    Returns:
        AngleDetectionResult: 包含角度檢測結果
    """
    start_time = time.time()
    
    try:
        logger.info("=== 開始獲取當前角度值 ===")
        ...
        # 步驟3: 發送角度檢測指令 (指令代碼5 - 僅檢測不校正)
        logger.info("發送角度檢測指令...")
        if not self._send_command_with_auto_clear_like_app(5):
            return AngleDetectionResult(
                success=False,
                message="發送角度檢測指令失敗"
            )
        ...
        # 步驟4: 等待檢測完成
        logger.info("等待角度檢測完成...")
        detection_result = self._wait_for_detection_completion()
        
        if detection_result.success:
            detection_time = time.time() - start_time
            logger.info(f"角度檢測成功，檢測角度: {detection_result.angle:.2f}度，耗時: {detection_time:.2f}秒")
...
def _wait_for_detection_completion(self) -> AngleDetectionResult:
    """等待角度檢測完成 (僅檢測，不校正)"""
    start_time = time.time()
    detection_timeout = 10.0  # 檢測超時10秒
    ...
    while time.time() - start_time < detection_timeout:
        try:
            ...
            logger.debug(f"檢測狀態: Ready={ready}, Running={running}, Alarm={alarm}")
            
            # 檢查是否有錯誤
            if alarm:
                return AngleDetectionResult(
                    success=False,
                    message="角度檢測過程發生錯誤，系統進入Alarm狀態"
                )
            
            # 檢查是否完成 (Ready=True且Running=False)
            if ready and not running:
                logger.info("角度檢測執行完成")
                ...
                if result_data and result_data.get('success', False):
                    return AngleDetectionResult(
                        success=True,
                        angle=result_data.get('detected_angle'),
                        message="角度檢測成功"
                    )
                else:
                    return AngleDetectionResult(
            ...
            logger.error(f"檢測狀態檢查異常: {e}")
            time.sleep(self.status_check_interval)
    
    logger.error(f"角度檢測執行超時 ({detection_timeout}秒)")
    return AngleDetectionResult(
        success=False,
        message=f"角度檢測執行超時 ({detection_timeout}秒)"
    )

def _read_detection_results(self) -> Optional[Dict[str, Any]]:
    """讀取角度檢測結果 (僅檢測，不包含校正資訊)"""
    try:
        result = self.modbus_client.read_holding_registers(
            address=self.base_address + 30, count=10, slave=1  # 檢測結果在不同地址
        )
        ...
        if angle_int >= 2**31:
            angle_int -= 2**32
        detected_angle = angle_int / 100.0
        
        return {
            'success': True,
            'detected_angle': detected_angle,
            'detection_count': registers[3],
            'detection_time': registers[4]
        }
        
    except Exception as e:
        logger.error(f"讀取檢測結果異常: {e}")
        return None
# ============================= 完美修正說明 ===============================
# 
# 🔥 關鍵修正項目：
