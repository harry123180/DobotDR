    ...
    def connect(self) -> bool:
        ...
        try:
            ...
            if self.client.connect():
                ...
                print(f"✅ Modbus TCP連接成功: {self.server_ip}:{self.server_port}")
...
class CCD1VisionController:
    ...
    def _save_detection_image(self, vis_image: np.ndarray, result: YOLODetectionResult):
        """保存檢測結果圖像到執行檔同層目錄"""
        try:
            # 獲取當前時間作為檔案名稱
            from datetime import datetime
            ...
            # 構建檔案名稱
            filename = f"CCD1_Detection_{timestamp}_F{result.dr_f_count}_S{result.stack_count}_M{result.model_id_used}.jpg"
            
            # 獲取執行檔同層目錄路徑
            save_path = os.path.join(self.working_dir, filename)
            
            # 保存圖像
            success = cv2.imwrite(save_path, vis_image)
            
            if success:
                print(f"💾 檢測結果圖像已保存: {filename}")
                print(f"   路徑: {save_path}")
                print(f"   檢測結果: DR_F={result.dr_f_count}, STACK={result.stack_count}, 模型={result.model_id_used}")
            else:
                print(f"❌ 圖像保存失敗: {filename}")
